...
public class RepositoryChainDependencyToComponentIdResolver implements DependencyToComponentIdResolver {
    ...
    public void resolve(DependencyMetadata dependency, ModuleIdentifier targetModuleId, BuildableComponentIdResolveResult result) {
        ...
        ModuleVersionConstraintInternal constraint = (ModuleVersionConstraintInternal) requested.getPreferredVersion();
        VersionSelector preferredSelector =
            constraint != null ? constraint.toVersionSelector(versionSelectorScheme) : versionSelectorScheme.parseSelector(requested.getVersion());
        ...
        if (result.hasResult()) {
            ModuleVersionConstraintInternal acceptConstraint = (ModuleVersionConstraintInternal) requested.getAcceptedVersion();
            VersionSelector acceptSelector = acceptConstraint != null ? acceptConstraint.toVersionSelector(versionSelectorScheme) : null;
            result.setSelectors(acceptSelector, acceptSelector);
        }
    }
